...
test('the summary page has a card per pair with status, score, locators and escaped names', () => {
  ...
  assert.match(html, /<code>#faq<\/code>/)
...
test('the pair page embeds the viewer, region rows and changed locators', () => {
  ...
  assert.match(html, /<elastishot-viewer baseline-src="pairs\/home--desktop\/baseline\.png" candidate-src="pairs\/home--desktop\/candidate\.png" diff-src="pairs\/home--desktop\/diff\.png" mode="slider" show-regions>/)
  ...
  assert.match(html, /Changed locators \(2\)/)

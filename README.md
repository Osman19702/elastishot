...
Elastishot is a Node 20+ library and CLI written in TypeScript. It is not on npm yet: install it from a checkout with `npm install <path-to-elastishot>` or from `npm pack`.
...
├── scripts/                          build-viewer, make-fixtures, check-layering, spike-opencv
...
├── docs/                             ATDD.md, ELEMENT-MAP.md, EMBEDDING.md
